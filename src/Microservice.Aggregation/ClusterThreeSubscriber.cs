...
internal sealed class ClusterThreeSubscriber([FromKeyedServices(Kafka.SubscriberThree)] IConsumer<string, AggregatedWeatherForecast> consumer, IServiceProvider serviceProvider, ILogger<ClusterThreeSubscriber> logger)
    : BaseHostedService(serviceProvider), IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        logger.LogInformation($"{nameof(ClusterThreeSubscriber)} started");
        consumer.Subscribe(TopicNames.ThreeConverterAggregator);
